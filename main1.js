let res = ['_','_','_','_','_','_','_','_','_','_','_','_','_','_'];
let quantity = localStorage.getItem('quantityNFL');
let name = localStorage.getItem("aliasNFL");
...
function start(){
    ...
    let results = localStorage.getItem("resultsNFL");
    if (results){
    ...
    document.getElementById("total").innerHTML = "Total: $" + quantity*50 +"\n";
...
function number(){
    quantity = localStorage.getItem("quantityNFL");
    if (quantity)
        localStorage.setItem('quantityNFL', ++quantity);
    else{
        localStorage.setItem('quantityNFL', 1);
        quantity = localStorage.getItem("quantityNFL");}
    document.querySelector('.botonenviar span').textContent = quantity;
    localStorage.setItem('aliasNFL', name);
}

function result(){
    results = localStorage.getItem("resultsNFL");
    name = document.getElementById("nombre").value;
    name  = name.split('*').join('');
    if (results){
        if (aux > 1)
        localStorage.setItem('resultsNFL', results + "\n" + res.join("\xa0\xa0") + "\xa0\xa0" + name + " (" + aux + ")" + "*");
        else
        ...
        {
            if (aux > 1)
            localStorage.setItem('resultsNFL',res.join("\xa0\xa0") + "\xa0\xa0" + name + " (" + aux + ")" +  "*");
            else    
                localStorage.setItem('resultsNFL',res.join("\xa0\xa0") + "\xa0\xa0" + name+ "*");}       
}
...
function send(){
    ...
    whatsapptext = encodeURI(localStorage.getItem("resultsNFL"));
...
function recovername(){
    name = localStorage.getItem("aliasNFL");
...
function calculate(){
    ...
    quantity = localStorage.getItem("quantityNFL");
    if (quantity){
        localStorage.setItem('quantityNFL', parseInt(quantity)+aux);
        quantity = localStorage.getItem('quantityNFL');}
    else{
        localStorage.setItem('quantityNFL', aux);
        quantity = localStorage.getItem("quantityNFL");}
    document.querySelector('.botonenviar span').textContent = quantity;
    localStorage.setItem('aliasNFL', name);
}
...
function costoactual(){
    if (!res.join("\xa0\xa0").includes("_")){
        ...
        document.getElementById("costo").innerHTML = "Costo: $" + aux2*50;
        document.getElementById("numquinielas").innerHTML = aux2 + " Quiniela(s)"
    }
}
function remove(){
    if (quantity > 0){
        x = localStorage.getItem("resultsNFL");
        x = removeLastLine(x);
        localStorage.setItem("resultsNFL", x);
        results = x.split("*");
        ...
        if (display.childElementCount < 2)
            localStorage.setItem("resultsNFL","");
